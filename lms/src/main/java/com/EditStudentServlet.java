...
public class EditStudentServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request,
		...
		try{
			...
			String stuimage = request.getParameter("stuimg");

			ResultSet r = con.createStatement().executeQuery("select * from student where stu_email='"+stuemail+"' and stu_id<>"+stuid);
			if(r.next()) {
		    ...
		    PreparedStatement ps = con.prepareStatement("update student set stu_name=?, stu_email=?, stu_phone=?, stu_occ=?, stu_img=? where stu_id=?");
		    ps.setString(1, stuname);
		    ps.setString(2, stuemail);
		    ps.setString(3, stuphone);
		    ps.setString(4, stuocc);
		    ps.setString(5, stuimage);
		    ps.setString(6, stuid);
		    
		    ps.executeUpdate();
		    session.setAttribute("msg", "Student profile updated successfully");
